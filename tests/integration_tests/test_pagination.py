...
async def test_first_page_has_no_prev(ac: AsyncClient):
    r = await ac.get(
        "/api/v1/hotels",
        params={"date_from": _DATE_FROM, "date_to": _DATE_TO, "page": 1, "per_page": 1},
    )
...
async def test_second_page_has_prev(ac: AsyncClient):
    ...
    r = await ac.get(
        "/api/v1/hotels",
        params={"date_from": _DATE_FROM, "date_to": _DATE_TO, "page": 2, "per_page": 1},
    )
    assert r.status_code == 200
...
async def test_last_page_has_no_next(ac: AsyncClient):
    ...
    r_all = await ac.get(
        "/api/v1/hotels", params={"date_from": _DATE_FROM, "date_to": _DATE_TO, "per_page": 100}
    )
...
async def test_pages_calculation(ac: AsyncClient):
    r = await ac.get(
        "/api/v1/hotels", params={"date_from": _DATE_FROM, "date_to": _DATE_TO, "per_page": 1}
    )
    data = r.json()
    import math
